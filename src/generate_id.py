...
class VehicleIDGenerator:
    def __init__(self, color_confidence_threshold=0.6, carname_confidence_threshold=0.7):
        ...
        # Persistent ID mapping: (color, car_name) -> vehicle_id
        # This ensures the same vehicle always gets the same ID
        self.vehicle_registry = {}
    
    def generate_id(self, color, car_name, color_conf, carname_conf):
        """
        Generate unique ID with confidence validation and persistent mapping
        ...
        # Create vehicle key for registry lookup
        vehicle_key = (color, car_name)

        # Check if we've seen this vehicle before
        if vehicle_key in self.vehicle_registry:
            # Return the previously assigned ID
            vehicle_id = self.vehicle_registry[vehicle_key]
            return vehicle_id, True

        # New vehicle: generate ID
        ...
        self.vehicle_registry[vehicle_key] = vehicle_id
    ...
    def get_registered_vehicles(self):
        """Get all registered vehicles and their IDs"""
        return dict(self.vehicle_registry)

    def get_registry_size(self):
        """Get the number of unique vehicles registered"""
        return len(self.vehicle_registry)